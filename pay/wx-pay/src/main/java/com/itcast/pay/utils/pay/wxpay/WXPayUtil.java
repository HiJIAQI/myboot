...
public class WXPayUtil {
     ...
     * 解密退款通知
     */
    public static Map<String, String> decodeRefundNotify(HttpServletRequest request) throws Exception {
        // 从request的流中获取参数
        Map<String, String> notifyMap = getNotifyParameter(request);
        String reqInfo = notifyMap.get("req_info");
        // 1.对加密串A做base64解码，得到加密串B
        byte[] bytes = WXPayUtil.decode(reqInfo);

        // 2.对商户key做md5，得到32位小写key*
        Cipher cipher = Cipher.getInstance(WXPayConstants.ALGORITHM_MODE_PADDING);
        SecretKeySpec key = new SecretKeySpec(WXPayUtil.MD5(WxMpConfig.MCHKEY).toLowerCase().getBytes(), WXPayConstants.ALGORITHM);
